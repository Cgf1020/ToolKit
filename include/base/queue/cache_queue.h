...
public:
	...
	*/
	bool Full() const {
		if (max_size_.load(std::memory_order_acquire) == 0) {
			return false;
		}
		std::lock_guard<std::mutex> l(cache_queue_mutex_);
		return data_queue_.size() >= max_size_.load(std::memory_order_relaxed);
	}
	...
	*/
	...
	bool Push(const T& t) {
		std::unique_lock<std::mutex> l(cache_queue_mutex_);
		const size_t max_size = max_size_.load(std::memory_order_relaxed);
		if (max_size > 0 && data_queue_.size() >= max_size) {
			return false;  // 队列已满
		}
		data_queue_.push(t);
	...
	void WaitAndPush(const T& t) {
		...
		if (max_size > 0) {
			cache_convar_.wait(l, [this] {
				const size_t ms = max_size_.load(std::memory_order_relaxed);
				return (ms == 0) || (data_queue_.size() < ms) || abort_;
			});
			if (abort_) return;
		}
		data_queue_.push(t);
...
private:
	...
	std::atomic<size_t> max_size_{ 0 };				//最大容量（0表示无限制）
};
}//namespace itflee